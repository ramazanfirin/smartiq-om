...
import com.smartiq.om.domain.User;
import com.smartiq.om.domain.enumeration.OrderStatus;
import com.smartiq.om.repository.AddressRepository;
import com.smartiq.om.repository.OrderRepository;
import com.smartiq.om.repository.UserRepository;
import com.smartiq.om.security.SecurityUtils;
...
class OrderResourceIT {
    ...
    public static Order createEntity(EntityManager em) {
        Order order = new Order().orderId(DEFAULT_ORDER_ID).status(DEFAULT_STATUS);
        return order;
    }
    ...
    public static Order createUpdatedEntity(EntityManager em) {
        Order order = new Order().orderId(UPDATED_ORDER_ID).status(UPDATED_STATUS);
        return order;
    }
    ...
    void putNewOrder() throws Exception {
        ...
        updatedOrder.orderId(UPDATED_ORDER_ID).status(UPDATED_STATUS);
    ...
    void fullUpdateOrderWithPatch() throws Exception {
        ...
        partialUpdatedOrder.orderId(UPDATED_ORDER_ID).status(UPDATED_STATUS);
    ...
    @Test
    @Transactional
    void completedOrder() throws Exception {
        // Initialize the database
        order.setStatus(OrderStatus.NEW);
        orderRepository.saveAndFlush(order);
        ...
        // Delete the order
        restOrderMockMvc
            .perform(get(ENTITY_API_URL + "/completed/" + order.getId()).accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());

        // Validate the database contains one less item
        List<Order> orderList = orderRepository.findAll();

        assertThat(orderList.get(0).getStatus()).isEqualTo(OrderStatus.COMPLETED);
    }
    ...
    void updateAddress() throws Exception {
        ...
        restOrderMockMvc
            .perform(post(ENTITY_API_URL + "/updateAddress/" + order.getId()).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(address)))
            .andExpect(status().isOk());

        // Validate the database contains one less item
        List<Order> orderList = orderRepository.findAll();

        assertThat(orderList.get(0).getAddress().getCity()).isEqualTo("city");
        assertThat(orderList.get(0).getAddress().getDistrict()).isEqualTo("district");
    }
}
